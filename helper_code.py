...
def find_recording_files(data_folder, patient_id):
    ...
    for file_name in sorted(os.listdir(patient_folder)):
        if not file_name.startswith('.') and file_name.endswith('.hea'):
            record_names.add(os.path.join(patient_folder, file_name))
...
def load_recording_data(record_name, check_values=False):
    ...
    if not signal_file.lower().endswith('.wav'):
        raise ValueError('Invalid file format. Only .wav files are supported.')

    # Load the audio data from the .wav file
    sampling_frequency, data = wavfile.read(signal_file)
    data = data.reshape(1, -1)

    # Perform additional checks and processing as needed
    ...
    return rescaled_data.ravel(), channels, sampling_frequency
...
def get_age(string):
    return get_variable(string, '#Age', str)
...
def get_weight(string):
    return get_variable(string, '#Weight', float)

def get_height(string):
    return get_variable(string, '#Height', float)

def get_murmur(string):
    return get_variable(string, "#Murmur", str)
